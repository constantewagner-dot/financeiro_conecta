...
export function ContasReceberPage() {
  ...
  const load = () => api.get('/financeiro/contas-receber').then((res) => setItems(res.data));
  useEffect(() => { load(); }, []);

  const save = async () => {
    await api.post('/financeiro/contas-receber', { ...form, valor: Number(form.valor) });
    toast.success('Conta criada');
    setForm({ descricao: '', valor: '', dataVencimento: '', categoriaId: '' });
    load();
  };

  const pagar = async (id: string) => { await api.put(`/financeiro/contas-receber/${id}/pagar`); toast.success('Pago'); load(); };
  const remove = async (id: string) => { if (confirm('Excluir?')) { await api.delete(`/financeiro/contas-receber/${id}`); toast.success('Excluído'); load(); } };
