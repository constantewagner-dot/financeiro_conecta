...
export function ProfessoresPage() {
  ...
  const load = () => api.get('/professores').then((res) => setItems(res.data));
  useEffect(() => { load(); }, []);

  const save = async () => {
    if (editId) { await api.put(`/professores/${editId}`, form); toast.success('Atualizado'); }
    else { await api.post('/professores', form); toast.success('Criado'); }
    setForm({ nome: '', email: '', telefone: '', cpf: '' });
    setEditId(null);
    load();
  };

  const edit = (p: any) => { setForm({ nome: p.nome, email: p.email || '', telefone: p.telefone || '', cpf: p.cpf || '' }); setEditId(p.id); };
  const remove = async (id: string) => { if (confirm('Excluir?')) { await api.delete(`/professores/${id}`); toast.success('Excluído'); load(); } };
          ...
          <tbody>
            {items.map((p) => (
              <tr key={p.id} className="border-t">
                ...
                <td className="px-4 py-3 text-sm">{p.cpf}</td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => edit(p)} className="p-2 text-blue-600 hover:bg-blue-50 rounded"><Edit className="h-4 w-4" /></button>
                  <button onClick={() => remove(p.id)} className="p-2 text-red-600 hover:bg-red-50 rounded"><Trash2 className="h-4 w-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
